...
export function Inspector({
  ...
  // iPad 坚持上文下图：不提供左右分栏的 split-landscape
  // mac-window 仅 macOS 设备可选；其他设备隐藏该 layout
  const layoutOptions = Object.entries(LAYOUT_LABEL).filter(([layout]) => {
    if (device === "feature-graphic") return layout === "feature-graphic";
    if (layout === "feature-graphic") return false;
    if (device === "ipad" && layout === "split-landscape") return false;
    if (layout === "mac-window") return device === "mac";
    return true;
  });
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-3">
        ...
        {/* Inverted：使用主题 bgAlt/fgAlt，与工具栏 Dark/Light 主色相反，用于节奏对比 */}
        {!isFeatureGraphic && (
          <div className="flex items-start justify-between gap-3 rounded-md border px-3 py-2.5">
            <div className="min-w-0 space-y-0.5">
              <Label htmlFor="slide-inverted" className="text-xs">
                Inverted
              </Label>
              <p className="text-[11px] leading-snug text-muted-foreground">
                Use the opposite background of the current Dark/Light theme.
              </p>
            </div>
            <button
              id="slide-inverted"
