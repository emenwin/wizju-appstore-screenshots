...
type Props = {
  ...
  /** 当前主题 id；Dark/Light 在 wizju-dark / wizju-light 间切换 */
  themeId: string;
...
export function Toolbar(props: Props) {
  ...
  // 按平台记住上次所选设备，切换 Tab 时恢复
  const lastByPlatform = React.useRef<{ ios: Device; android: Device; macos: Device }>({
    ...
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5 border-b bg-card/40 px-4 py-2">
      ...
      <Tabs
        ...
        onValueChange={(p) => {
          ...
          const key = p as "ios" | "android" | "macos";
          props.setDevice(lastByPlatform.current[key]);
        }}
      >
      ...
      {/* Dark / Light：映射到 wizju 品牌主题，驱动幻灯片画布配色 */}
      <Tabs
        value={isLightThemeId(props.themeId) ? "light" : "dark"}
        onValueChange={(mode) => {
          if (props.busy) return;
          props.setThemeId(mode === "light" ? WIZJU_THEME.light : WIZJU_THEME.dark);
        }}
      >
        <TabsList className="h-8 p-0.5">
          <TabsTrigger value="dark" className="h-7 px-3 text-xs" disabled={props.busy}>
            Dark
          </TabsTrigger>
          <TabsTrigger value="light" className="h-7 px-3 text-xs" disabled={props.busy}>
            Light
          </TabsTrigger>
        </TabsList>
      </Tabs>
