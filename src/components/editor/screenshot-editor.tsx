  ...
  const assetPaths = React.useMemo(() => {
    ...
    for (const s of allSlides) {
      for (const raw of [s.screenshot, s.screenshotSecondary]) {
        ...
        // 预加载所有 locale × orientation 变体，避免横竖屏切换或批量导出时抢图
        const needsLocale = raw.includes("{locale}");
        const needsOrientation = raw.includes("{orientation}");
        if (needsLocale || needsOrientation) {
          const locales = needsLocale ? state.locales : [state.locale];
          const orients = needsOrientation ? (["portrait", "landscape"] as const) : [state.orientation];
          for (const loc of locales) {
            for (const orient of orients) {
              paths.add(resolveScreenshot(raw, loc, orient));
            }
          }
        } else {
          paths.add(raw);
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-background">
      ...
      <Toolbar
        ...
        setOrientation={(v) =>
          setState((p) => {
            if (p.orientation === v) return p;
            // 横竖屏画布尺寸不同：清掉元素 transforms，回到 layout 默认上文下图位置
            const slides = (p.slidesByDevice[p.device] || []).map((s) => ({
              ...s,
            ...
            return {
              ...p,
              orientation: v,
              slidesByDevice: { ...p.slidesByDevice, [p.device]: slides },
            };
          })
        }
