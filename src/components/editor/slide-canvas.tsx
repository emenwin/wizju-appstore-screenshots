...
export function getCanvas(device: Device, orientation: Orientation) {
  ...
  // iPad / Android 平板：横屏使用 wL×hL 画布
  if (
    orientation === "landscape" &&
    (device === "ipad" || device === "android-7" || device === "android-10") &&
    c.wL &&
    c.hL
  ) {
...
function getFrameAspect(device: Device, orientation: Orientation) {
  switch (device) {
    ...
    case "ipad":        return orientation === "landscape" ? IPAD_L_RATIO : IPAD_RATIO;
    case "mac":         return MACBOOK_RATIO;
    case "android-7":
...
export function getFrameForDevice(device: Device, orientation: Orientation): {
  ...
  switch (device) {
    ...
    case "ipad":
      // 横屏用 IPadLandscape + ipadLW，默认上文下图（hero / device-bottom）
      if (orientation === "landscape") {
        return {
          Comp: IPadLandscape,
          widthFn: ipadLW,
          smallWidthFn: (cW, cH) => ipadLW(cW, cH, 0.48),
        };
      }
      return { Comp: IPad, widthFn: ipadW, smallWidthFn: (cW, cH) => ipadW(cW, cH, 0.6) };
    case "mac":
      // 默认 MacBook 整机；mac-window 布局在 getSlideGeometry 中切换为 MacWindow
      return {
        Comp: MacBook,
        widthFn: macW,
        smallWidthFn: (cW, cH) => macW(cW, cH, 0.62),
      };
    case "android":
...
function getDefaultRects(
  ...
  switch (layout) {
    ...
    // mac-window：文案在上、窗口居中偏下（几何同 device-bottom，机框由 getSlideGeometry 切换）
    case "mac-window":
      return {
        caption: { x: cW * 0.08, y: cH * 0.07, width: capW, height: capH, align: "center" },
        device: {
          x: (cW - deviceW) / 2,
          y: cH - deviceH - cH * 0.04,
          width: deviceW,
          height: deviceH,
        },
      };
...
function getSlideGeometry(slide: Slide, device: Device, orientation: Orientation) {
  ...
  let { Comp: Frame, widthFn, smallWidthFn } = getFrameForDevice(device, orientation);
  let frameAspect = getFrameAspect(device, orientation);

  // mac-window：改用窗口机框与对应宽高比，不再使用 MacBook 整机
  if (slide.layout === "mac-window" && device === "mac") {
    Frame = MacWindow;
    frameAspect = MAC_WINDOW_RATIO;
    widthFn = macWindowW;
    smallWidthFn = (w, h) => macWindowW(w, h, 0.58);
  }
...
function SlideElements({
  ...
  const screenshot = resolveScreenshot(slide.screenshot, locale, orientation);
  const screenshotSecondary = resolveScreenshot(slide.screenshotSecondary, locale, orientation);
